...
/**
 * Java虚拟机规范.Java SE 8版 285页
 */
public class invokestatic implements Opcode {
    ...
    public void opt(Frame frame) {
        ...
        if (Arrays.asList(methodInfo.getAccessFlag()).contains("ACC_NATIVE")) {
            _invoke.invokeNative(methodInfo, parameterClazzArray, frame.getOperandStack(), false);
        } else {
            // 新增栈帧执行方法
            Frame newFrame = new Frame(frame,
                    _invoke.getLocalVariableByParamClazzArray(new Long[methodInfo.getCodeAttribute().getMaxLocals().getValue()], parameterClazzArray, frame.getOperandStack(), false),
                    new Stack<>(),
